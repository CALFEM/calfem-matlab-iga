...
%% Do degre elevation refinement

% Raise degree by:
d = 2; 

Pw = [x.*w y.*w w];            % We do the refinement by treating the 2D NURBS
                               % as a 3D B-Spline and use the B-Spline refinement
                               % function:
...
plot(Xi_store,R)
grid on
title('NURBS Basis functions after degree elevation')
xlabel('$\xi$','interpreter','latex')
...
plot(C2(1,:),C2(2,:))
hold on
title('NURBS curve after degree elevation')
xlabel('x')
ylabel('y')
